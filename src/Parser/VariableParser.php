...
use type Ytake\Dotenv\Exception\ParseException;
...
class VariableParser extends AbstractParser {
  ...
  const string REGEX_ENV_VARIABLE = '\\${(.*?)}';

  const string SYMBOL_ASSIGN_DEFAULT_VALUE = '=';

  const string SYMBOL_DEFAULT_VALUE = '-';

  public function __construct(
    Parser $parser, 
    private array<string, mixed> $context = []
  ) {
    ...
    private function fetchVariableMatches(string $value): Map<int, mixed> {
      $matches = [];
